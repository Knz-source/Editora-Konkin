...
contract TestKIN is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    // Editor address
	// hardhat network test addr 0x8626f6940E2eb28930eFb4CeF49B2d1F2C9C1199
    address EDITOR_ADDRESS = 0x8626f6940E2eb28930eFb4CeF49B2d1F2C9C1199;

	// Liquidity address
	// hardhat network test addr 0xdD2FD4581271e230360230F9337D5c0430Bf44C0
	address LIQUIDITY_ADDRESS = 0xdD2FD4581271e230360230F9337D5c0430Bf44C0;
	...
	string private _name = "Test KIN";
	string private _symbol = "TKIN";
	...
	constructor (address Editor, address Liquidity) {
		EDITOR_ADDRESS = Editor;
		LIQUIDITY_ADDRESS = Liquidity;
		_rOwned[_msgSender()] = _rTotal;

		//pancake v2 testnet 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
		...
		// Excludes the EDITOR and LIQUIDITY adress from fee and dividends rewards
	...
	function deliver(uint256 tAmount) public {
		...
		_rOwned[sender] = _rOwned[sender].sub(rValues.Amount);
		_rTotal = _rTotal.sub(rValues.Amount);
	...
	function _getTValues(uint256 tAmount) private view returns (TxValues memory) {
		...
		tValues.TransferAmount = tAmount.sub(tValues.LiquidityFee).sub(tValues.HolderFee).sub(tValues.Burn).sub(tValues.EditorFee);
		return tValues;
	}
	...
	function _getRValues(uint256 tAmount, uint256 tEditorFee, uint256 tHolderFee, uint256 tLiquidityFee, uint256 tBurn, uint256 currentRate) private pure returns (TxValues memory) {
		...
		rValues.LiquidityFee = tLiquidityFee.mul(currentRate);
		rValues.Burn = tBurn.mul(currentRate);
		rValues.TransferAmount = rValues.Amount.sub(rValues.LiquidityFee).sub(rValues.HolderFee).sub(rValues.Burn).sub(rValues.EditorFee);
		return rValues;
	}
